...
const Update = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:3001/getalldata/${id}`)
            .then(result => {
                console.log(result);
                setName(result.data.name);  // Prevent undefined issues
        ...
        <div>
            <div className='mt-20 mx-auto my-auto w-1/3 h-[80vh] bg-gray-800 flex flex-col items-center p-5 '>
                <h2 className='mb-5 p-3 text-4xl text-red-500 font-serif mx-auto'>
                    <span className='p-2 bg-teal-200 rounded'>Modify User</span>
                </h2>
                <form onSubmit={handleUpdate} className='flex-1 justify-center items-center'>
                    <div>
                    ...
                    <div>
                        <button type='submit' on className='mt-5 p-2 rounded-md text-white text-center font-serif bg-green-700'>
                            Modify
                        </button>
                    </div>
                </form>
